...
NinjaDBG v1.1.2 is a CLI-only stealth debugger for Linux x86-64. The GUI
was removed in v1.1.2, so there are no Cairo, Pango, or X11 dependencies.
...
This produces `build/ninjadb_v1.1.2_amd64.deb`. Install it with:

```bash
sudo apt-get install ./build/ninjadb_v1.1.2_amd64.deb
# or, equivalently:
sudo dpkg -i build/ninjadb_v1.1.2_amd64.deb
```
