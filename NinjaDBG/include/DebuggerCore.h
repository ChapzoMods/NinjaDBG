...
public:
    ...
    // v1.0.4 — conditional breakpoints: only fire when condition evaluates true
    ...
    // v1.0.4 — temporary breakpoints (auto-remove after first hit)
    int  addTempBreakpoint(addr_t addr, const std::string& label = "_temp");

    // v1.0.4 — watchpoints (memory access breakpoints via DR0-DR3 + DR7)
    ...
    // v1.0.4 — step out: continue until current function returns
    // (sets a temp bp at the return address on top of stack)
    bool stepOut();

    // v1.0.4 — step over: if current instruction is a CALL, set temp bp after it
    bool stepOver();

    // v1.0.4 — run until next syscall entry/exit (PTRACE_SYSCALL)
    bool stepToSyscall(int& syscall_nr, bool& is_entry);

    // v1.0.4 — backtrace (walks RBP chain, returns instruction pointers)
    struct Frame {
        addr_t rip;
        addr_t rbp;
        addr_t rsp;
    ...
    // v1.0.4 — get current syscall number (from orig_rax)
...
private:
    ...
    std::vector<MemoryRegion> maps_cache_;   // v1.0.4: cached for backtrace symbol resolution
